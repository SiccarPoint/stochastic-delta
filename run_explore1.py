...
areas = (0.1, )  # (0.025, 0.05, 0.1, 0.2, 0.4, 0.8)
lo_areas = (0.025, )  # (0.025, 0.05, 0.1)
hi_areas = (0.2,  )  # (0.2, 0.4, 0.8)
...
for area in which_areas:
    for drift in drifts:
        ...
        for i in xrange(iterations):
            ...
            completeness_records=completeness_walk, graphs=True, Q=Q_real,       ####False!!!
            walking_erosion_depo=True)
            num_roll_pres_walk.append(mydelta.final_preserved.sum())

        if which_one is 'lo':
            completenesses_loarea[area][drift] = copy(completeness_walk)
            num_roll_pres_loarea[area][drift] = copy(num_roll_pres_walk)
        else:
            completenesses_hiarea[area][drift] = copy(completeness_walk)
            num_roll_pres_hiarea[area][drift] = copy(num_roll_pres_walk)
