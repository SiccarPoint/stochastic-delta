...
Q_options = [np.loadtxt('SPU5e-4Acc10dt200_12.30.44_sedfluxout.txt'), ]
Q_options_sde = [np.loadtxt('U5e-4Acc10dt200_13.40.11_sedfluxout.txt'), ]
for SP, SDE in zip(Q_options, Q_options_sde):
    ratio = SDE[0]/SP[0]
    SP *= ratio
accel_options = [10., ]
Uinit = 0.0005
flux_scaling = 0.4  # arbitrary accounting for basin size
...
for (Q_in, accel) in zip(Q_options, accel_options):
    ...
    # NB: Flux accel begins after 400 tsteps, i.e., halfway through in this
    # model. The transient is about 200 steps long.
